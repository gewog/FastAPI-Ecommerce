...
async def update_rating(session: AsyncSession, product_id: int) -> None:
    """Функция подсчета и изменения рейтинга продукта."""
    ...
    if average_rating is not None:
        update_stmt = (
            ...
            .values(rating=average_rating)  # Исправлено: обновляем поле `rating`, а не `id`!
        )
...
@router.get("/all_reviews",
            summary="Метод получения всех отзывов и рейтингов о товарах")
async def all_reviews(session: session):
    ...
    if scal_res:
        for el in scal_res:
            ...
            comments[el.id] = {"product": product.name,
                                "rating": el.rating,
                                "comment": el.comment,
...
@router.patch("/delete_reviews",
              summary="Метод удаления отзыва и рейтинга об определенном товаре")
async def delete_reviews(session:session,
    ...
    check_query = await session.execute(
        select(Review).filter_by(id = review_id)
    )
